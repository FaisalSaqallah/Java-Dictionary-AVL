...
public class SLL<T> {
    ...
    public String[] toArray(int size) {
        ...
        SLLNode<String> p = (SLLNode<String>) head;
    ...
    public void insertBefore(int index, T newElem) throws Exception {
        if (isEmpty())
            ...
            SLLNode<T> tmp= new SLLNode<T>(newElem);
            head = tmp;
        } else {
            SLLNode<T> tmp = head;
            SLLNode<T> o = new SLLNode<>(newElem);
            ...
            public void insertAfterSecondOccurrence(T e1, T e2) throws Exception{
                if (isEmpty())
                    ...
                    SLLNode<T> tmp = head;
                    SLLNode<T> o = new SLLNode<T>(e1);
                    int count = 0;
        
                    while(tmp.next != null & count<2){
                        if(tmp.info == e2)
                            count++;
                        if (count < 2)
                            tmp = tmp.next;
                        ...
                        if(count <2)
                            throw new Exception("there is no second occurrence .");
                        else{
                        o.next = tmp.next;
                        tmp.next = o;
                        }
